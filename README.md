...
**Order/Pay/Delivery/MyPage서비스 주문정보 조회**
```
...
Correlation Key를 활용하기 위해 Id를 Key값으로 사용하였으며 주문된 동일한 Id로 취소한다.

주문 취소가 되면 OrderStatus가 cancelled로 Update 되는 것을 볼 수 있다.
...
```
- 비동기식 호출은 다른 서비스가 비정상이여도 이상없이 동작가능하여, Pay 서비스에 장애가 발생하여도 Order 서비스는 정상 동작을 확인
